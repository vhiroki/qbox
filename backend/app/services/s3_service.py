...
class S3Service:
    ...
    def _get_s3_client(self, connection_id: str):
        ...
        session_kwargs: dict[str, Any] = {"region_name": config.get("region", "us-east-1")}
                    ...
                    files.append(
                        {
                            ...
                            "extension": file_ext,
                            "is_structured": file_ext
                            in ["parquet", "csv", "xlsx", "xls", "json", "jsonl"],
                        }
                    )
    ...
    async def get_file_metadata(self, connection_id: str, file_path: str) -> dict[str, Any]:
        """
            ...
            try:
                ...
                try:
                    if file_ext == "parquet":
                        ...
                        count_query = (
                            f"SELECT COUNT(*) as count FROM read_csv('{s3_path}', AUTO_DETECT=TRUE)"
                        )
                    elif file_ext in ["json", "jsonl"]:
    ...
    async def create_file_view(
        self, connection_id: str, file_path: str, view_name: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            if file_ext == "parquet":
                create_query = (
                    f"CREATE OR REPLACE VIEW {view_name} AS SELECT * FROM read_parquet('{s3_path}')"
                )
            elif file_ext == "csv":
